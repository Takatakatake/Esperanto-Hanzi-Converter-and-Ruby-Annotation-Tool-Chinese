...
# #### __エスペラント文の文字列(漢字)置換のために最小限実行する必要がある範囲１(始まり)__
...
def measure_text_width_Arial16(text, char_widths_dict: Dict[str, int] = char_widths_dict) -> int:
    """
...
# for j in ["kombin","(を)組合わせる","esperant","エスペラント"]:
#     width_px = measure_text_width_Arial16(j, char_widths_dict)
#     print(f"文字列: {j}",f",幅: {width_px}px")
# for i in list("abcdefghijklmnopqrstuvwxyz"):
#     print(i,': ',measure_text_width_Arial16(i, char_widths_dict))


# HTML形式におけるルビ文字列内での改行のために、適切な位置に"<br>"を挿入する関数
def insert_br_at_half_width(text, char_widths_dict: Dict[str, int] = char_widths_dict) -> str:
    """
...
# 使用例
# test_cases = ["abcdefghi", "mmmmmmmmmm", "","a","ab", "abc", "aab","受動完了","接尾辞過去形"]
# for i in test_cases:
#     print(insert_br_at_half_width(i))

# HTML形式におけるルビ文字列内での改行のために、適切な位置に"<br>"を"2つ"挿入する関数
def insert_br_at_third_width(text, char_widths_dict: Dict[str, int] = char_widths_dict) -> str:
    """
    与えられた文字列の Arial16px 形式における文字幅を計算し、
...
# test_cases = ["abcdefghi", "mmmmmmmmmm", "","a","ab", "abc", "aab","受動完了","接尾辞過去形"]
# for i in test_cases:
#     print(insert_br_at_third_width(i))
...
def output_format(main_text, ruby_content, format_type, char_widths_dict: Dict[str, int] = char_widths_dict) -> str:
...
# #### __エスペラント文の文字列(漢字)置換のために最小限実行する必要がある範囲１(終わり)__
...
# まず、"世界语全部单词列表_约44700个(原pejvo.txt)_utf8转换_第二部分以后重点修正_追加2024年版PEJVO更新项目_最终版202501.txt"を小文字・字上符形式に変換する。 (変換コード上では小文字・字上符形式で統一している。)
...
with open("一时的_PEJVO(世界语全部单词列表)を小文字・字上符形式に转换.txt", 'r', encoding='utf-8') as file:
    ...
    for line in file:
        ...
        E_wordS = E_wordS.lstrip('/')# 日本版,第一部分だけ
        ...
        for jj in range(len(E_wordS_list)):
            if jj>=0:# 第一部分と第二部分以後も('全部')
                ...
                if not (contains_digit(E_word) or len(E_word)<2):# 2文字以上 かつ　数字を含まない単語
                    if "/" in E_word:
                        E_stem_w_partofspeech=["/".join(E_word.split("/")[:-1])]
                        if E_word.endswith(('/o','/on','oj','/o!','ojn','on!')):
                            E_stem_w_partofspeech.append('名词')
                        elif E_word.endswith(('/a','/aj','/an','/an!')):
                            E_stem_w_partofspeech.append('形容词')
                        elif E_word.endswith(('/e','/e!')):
                            E_stem_w_partofspeech.append('副词')
                        elif E_word.endswith(('/e/n','/e/n!')):# '/e/n'は後で気をつける  圧倒的に少ない(72個)ので無詞にしたほうが(20240707確認)
                            E_stem_w_partofspeech=[E_word,"無詞"]
                        elif E_word.endswith(('/i','/u','/u!')):
                            E_stem_w_partofspeech.append('动词')
                        elif E_word.endswith(('/n')):
                            E_stem_w_partofspeech.append('n词')            
                    else:
                        E_stem_w_partofspeech=[E_word,"无词"]
                    E_stem_with_Part_Of_Speech_list.append(E_stem_w_partofspeech)
...
with open("PEJVO(世界语全部单词列表)'全部'について、词尾(a,i,u,e,o,n等)をcutし、comma(,)で隔てて词性と併せて记录した列表(E_stem_with_Part_Of_Speech_list).json", "w", encoding="utf-8") as g:
    json.dump(E_stem_with_Part_Of_Speech_list, g, ensure_ascii=False, indent=2)  # ensure_ascii=FalseでUnicodeをそのまま出力
...
import json
# JSONデータを読み込む
with open("PEJVO(世界语全部单词列表)'全部'について、词尾(a,i,u,e,o,n等)をcutし、comma(,)で隔てて词性と併せて记录した列表(E_stem_with_Part_Of_Speech_list).json", "r", encoding="utf-8") as g:
    E_stem_with_Part_Of_Speech_list = json.load(g)
len(E_stem_with_Part_Of_Speech_list)
...
# ただし、その完璧な精度の置換のためにはあらかじめ"世界语全部单词列表_约44700个(原pejvo.txt)_utf8转换_第二部分以后重点修正_追加2024年版PEJVO更新项目_最终版202501.txt"から"世界语全部单词列表_约44700个(原pejvo.txt)_utf8转换_第二部分以后重点修正_追加2024年版PEJVO更新项目_最终版202501.txt＿から＿世界语全部词根_约11137个_202501.txt＿を抽出.ipynb"を用いてエスペラントの全語根を抽出しておく必要がある。

# 一旦辞書型を使う。(後で内容(value)を更新するため)
temporary_replacements_dict={}
with open("世界语全部词根_约11137个_202501.txt", 'r', encoding='utf-8') as file:
...
# 上のセルでの作業に引き続き、"'PEJVO(世界语全部单词列表)'全部'について、词尾(a,i,u,e,o,n等)をcutし、comma(,)で隔てて词性と併せて记录した列表'(E_stem_with_Part_Of_Speech_list)を文字列(漢字)置換するための、置換リスト"を作成していく。　
# ここでは置換リスト(辞書型配列)に自分で作成したエスペラント語根の日本語訳ルビリストを反映させる。
import pandas as pd
from io import StringIO

# ⇓以下のcsvファイルの内容を色々カスタマイズすることで、様々な内容の文字列(漢字)置換が可能になる。
input_csv_file="Mingeo_hanzi_hua.csv"
...
#                 temporary_replacements_dict[E_root]=[output_format(E_root, hanzi_or_meaning, format_type, char_widths_dict),len(E_root)]# 辞書型配列では要素(key)に対する値(value)を後から更新できることを利用している。
...
# '置換リスト'の各置換に対してplaceholder(占位符)を追加し、リスト'temporary_replacements_list_final'として完成させる。 このリストは、以降のコードでも頻繁に用いる。
...
imported_placeholders_for_global_replacement = import_placeholders('占位符(placeholders)_$20987$-$499999$_全域替换用.txt')
...
# リスト'temporary_replacements_list_final'("'PEJVO(世界语全部单词列表)'全部'について、词尾(a,i,u,e,o,n等)をcutし、comma(,)で隔てて词性と併せて记录した列表'(E_stem_with_Part_Of_Speech_list)を文字列(漢字)置換するための、一時的な置換リスト"の完成版)の内容確認
...
# ### __⇓は１番処理に時間がかかるセル。　最下部のセルでpythonの'multiprocessing'による並列化を実装している__
...
for j in E_stem_with_Part_Of_Speech_list:# 20秒ほどかかる。　先にリストの要素を全て結合して、一つの文字列にしてから置換する方法を試しても(上述)、さほど高速化しなかった。
...
# ここから、"'PEJVO(世界语全部单词列表)'全部'について、词尾(a,i,u,e,o,n等)をcutし、comma(,)で隔てて词性と併せて记录した列表'(E_stem_with_Part_Of_Speech_list)をリスト'temporary_replacements_list_final'(一時的な置換リストの完成版)によって文字列(漢字)置換し終えたリスト(辞書型配列)"(pre_replacements_dict_1)を最終的な置換リスト(replacements_final_list)に成形していく。
...
# #### エスペラント文を漢字置換する際
# 
# エスペラント文を漢字置換する際、CSVファイルに登録されている __「対応する漢字が定義されているエスペラント語根」__ の数は、 __「訳ルビを付与するために意味が定義されているエスペラント語根」__ の数よりも、往々にして少なくなりがちです。その結果、漢字置換の精度が下がってしまうケースがあることが判明しました。
# 
# #### 便宜上の一時置換
# 
# そこで、漢字置換の対象には含まれないものの、訳ルビを付与するための意味が定義されているエスペラント語根については、まず __便宜上、一時的に「置換前と同じ文字列」に置き換える__ 手法をとることにしました。これによって、訳ルビを振る際とまったく同じ置換優先順位を適用できるようになるからです。そして最終的に、 __「便宜上、一時的に置換前と同じ文字列に漢字置換した箇所」__ を __元の文字列に戻す__ 、という手法を採用しています。
# 
# #### 下記の関数について
# 
# 下記の関数は、この __「便宜上、一時的に置換前と同じ文字列に漢字置換した箇所」__ を、 __最終的に元の文字列へ復元する__ ために用いるものです。
...
# (1) 正規表現パターンをコンパイル
#     <ruby> {1} <rt class="XXL_L"> {2} </rt> </ruby>
#     のパターンにマッチさせる。{1}, {2} には "<" を含まない前提とする。
IDENTICAL_RUBY_PATTERN = re.compile(r'<ruby>([^<]+)<rt class="XXL_L">([^<]+)</rt></ruby>')

def remove_redundant_ruby_if_identical(text: str) -> str:## この'remove_redundant_ruby_if_identical' は '大文字'、'小文字'、'文頭だけ大文字'の3パターンに増えてしまう直前に発動。
    """
...
test_string = """
これはテストです。<ruby>ABC<rt class="XXL_L">ABC</rt></ruby> の部分は置換されるはず。
しかし <ruby>DEF<rt class="XXL_L">Def</rt></ruby> は大文字小文字が異なるので置換されません。
そして <ruby>GHI<rt class="XXL_L">GHI</rt></ruby> も置換されます。
数字の例: <ruby>123<rt class="XXL_L">123</rt></ruby> は同じなので置換される。
だけど <ruby>123<rt class="XXL_L">0123</rt></ruby> は文字列が異なるので置換されない。
1文字の例: <ruby>A<rt class="XXL_L">A</rt></ruby> は置換される。
小文字と大文字: <ruby>a<rt class="XXL_L">A</rt></ruby> は置換されない。
<ruby>ABC<rt class="XXL_L">ABC</rt></ruby>
<ruby>DEF<rt class="XXL_L">DEF</rt></ruby>
<ruby>ghi<rt class="XXL_L">ghi</rt></ruby>
<ruby>ABC <rt class="XXL_L">ABC</rt></ruby> このパターンはどうなる？
<ruby>XYZ<rt class="XXL_L">XYZ </rt></ruby> スペースが末尾にあるとどうなる？
"""
result = remove_redundant_ruby_if_identical(test_string)
print(result)
...
safe_replace('an', temporary_replacements_list_final)
...
AN_replace_str = safe_replace('an', temporary_replacements_list_final)
AN_treatment=[]

# pre_replacements_dict_3内での重複の検査をし、それを元に、以下のセルにおける修正がなされている。
pre_replacements_dict_3={}
# 辞書をコピーする
pre_replacements_dict_2_copy = pre_replacements_dict_2.copy()# これがあるので、再実行するときは2個前のセルに戻ってpre_replacements_dict_2を作り直してからでないといけない。
for i,j in pre_replacements_dict_2_copy.items():# j[0]:置換後の文字列　j[1]:品詞 j[2]:置換優先順位
    if i.endswith('an') and (AN_replace_str in j[0]) and ("名词" in j[1]) and (i[:-2] in pre_replacements_dict_2_copy):# and ("形容词" in pre_replacements_dict_2_copy[i[:-2]][1]) 190個→121個
...
# print("overlap_1_c   = ", overlap_1)
# print("overlap_2_c   = ", overlap_2)
# print("overlap_2_2_c = ", overlap_2_2)
# print("overlap_2_3_c = ", overlap_2_3)
# print("overlap_3_c   = ", overlap_3)
# print("overlap_4_c   = ", overlap_4)
# print("overlap_5_c   = ", overlap_5)
# print("overlap_6_c   = ", overlap_6)
# print("overlap_7_c   = ", overlap_7)
# print("overlap_8_c   = ", overlap_8)
# print("overlap_9_c   = ", overlap_9)
# print(overlap_10)
# print(overlap_11)
# print(overlap_12)
# print(AN_treatment)
...
# 比較のため、正規表現  <[^>]*> でHTMLタグをすべて消去
# ===== 1つ目の Python 出力から得られた値 ===== 字上符形式に変えたとき
...
# ===== 2つ目の Python 出力から得られた値 ===== X形式のとき(比較のため、X形式から字上符形式に変換)
...
# ===== 3つ目の Python 出力から得られた値 ===== 2024年版PEJVOの更新分を追加したとき
overlap_1_c   =  [['buro', 'buro', 'buro'], ['haloo', 'haloo', 'haloo'], ['taŭro', 'taŭro', 'taŭro'], ['unesko', 'unesko', 'unesko']]
overlap_2_c   =  [['alo', 'aloアロエ', 'al~の方へo'], ['duon', 'du二on分数', 'du二on'], ['okon', 'ok八on分数', 'ok八on']]
overlap_2_2_c =  [['sia', 'sia', 'si自分a'], ['eman', 'eman', 'em傾向an'], ['lian', 'lian[植]つる植物', 'li彼an'], ['pian', 'pian[楽]ピアノ', 'pi信心深いan']]
overlap_2_3_c =  []
overlap_3_c   =  [['agat', 'agat[鉱]メノウ', 'ag行動するat受動継続'], ['agit', 'agit(を)扇動する', 'ag行動するit受動完了'], ['amas', 'amas集積;大衆', 'am愛するas現在形'], ['iris', 'iris[解]虹彩', 'ir行くis過去形'], ['irit', 'irit', 'ir行くit受動完了']]
overlap_4_c   =  [['aĝi', 'aĝi打ち歩', 'aĝ年齢i']]
overlap_5_c   =  [['nombron', 'nombr数on分数', 'nombr数on'], ['patron', 'patron後援者', 'patr父on'], ['karbon', 'karbon[化]炭素', 'karb炭on'], ['ciklon', 'ciklon低気圧', 'cikl周期on'], ['aldon', 'al~の方へdon与える', 'aldアルトon'], ['balon', 'balon気球', 'bal舞踏会on'], ['baron', 'baron男爵', 'bar障害on'], ['baston', 'baston棒', 'bast[植]じん皮on'], ['magneton', 'magnet[理]磁石on分数', 'magnet[理]磁石on'], ['beton', 'beton', 'betビートon'], ['bombon', 'bombonキャンデー', 'bomb爆弾on'], ['breton', 'breton', 'bret棚on'], ['burĝon', 'burĝon芽', 'burĝブルジョワon'], ['centon', 'cent百on分数', 'cent百on'], ['milon', 'mil千on分数', 'mil千on'], ['kanton', 'kanton(フランスの)郡', 'kant(を)歌うon'], ['citron', 'citron[果]シトロン', 'citr[楽]チターon'], ['platon', 'platon', 'plat平たいon'], ['dekon', 'dek十on分数', 'dek十on'], ['kvaron', 'kvar四on分数', 'kvar四on'], ['kvinon', 'kvin五on分数', 'kvin五on'], ['seson', 'ses六on分数', 'ses六on'], ['trion', 'tri三on分数', 'tri三on'], ['karton', 'karton厚紙', 'kartカードon'], ['foton', 'fot写真を撮るon分数', 'fot写真を撮るon'], ['peron', 'peron階段', 'perよってon'], ['elektron', 'elektr電気on分数', 'elektr電気on'], ['drakon', 'drakon', 'drak竜on'], ['mondon', 'mon金銭don与える', 'mond世界on'], ['pension', 'pension下宿屋', 'pensi年金on'], ['ordon', 'ordon(を)命令する', 'ord順序on'], ['eskadron', 'eskadron', 'eskadr[軍]艦隊on'], ['senton', 'sen(~)なしでton[楽]楽音', 'sent(を)感じるon'], ['eston', 'eston', 'est(~)であるon'], ['fanfaron', 'fanfaron大言壮語する', 'fanfar[楽]ファンファーレon'], ['fero', 'fero', 'fer鉄o'], ['feston', 'feston花綱', 'fest(を)祝うon'], ['flegmon', 'flegmon', 'flegm冷静on'], ['fronton', 'fronton[建]ペディメント', 'front正面on'], ['galon', 'galon[服]モール', 'gal[生]胆汁on'], ['mason', 'mason築く', 'masかたまりon'], ['helikon', 'helikon', 'helik[動]カタツムリon'], ['kanon', 'kanon[軍]大砲', 'kan[植]アシon'], ['kapon', 'kapon去勢オンドリ', 'kap頭on'], ['kokon', 'kokon[虫]繭(まゆ)', 'kokニワトリon'], ['kolon', 'kolon[建]円柱', 'kol[解]首on'], ['komision', 'komision(調査)委員会', 'komisi(を)委託するon'], ['salon', 'salonサロン', 'sal塩on'], ['ponton', 'ponton[軍]平底舟', 'pont橋on'], ['koton', 'koton綿', 'kot泥on'], ['kripton', 'kripton', 'kript[宗]地下聖堂on'], ['kupon', 'kuponクーポン券', 'kup吸い玉on'], ['lakon', 'lakon', 'lakラッカーon'], ['ludon', 'lu賃借するdon与える', 'lud(を)遊ぶon'], ['melon', 'melon[果]メロン', 'melアナグマon'], ['menton', 'menton[解]下あご', 'ment[植]ハッカon'], ['milion', 'milion百万', 'mili[植]キビon'], ['milionon', 'milion百万on分数', 'milion百万on'], ['naŭon', 'naŭ九on分数', 'naŭ九on'], ['violon', 'violon[楽]バイオリン', 'viol[植]スミレon'], ['refoj', 're再びfoj回', 'refリーフoj'], ['trombon', 'trombon[楽]トロンボーン', 'tromb[気]たつまきon'], ['samo', 'samo', 'sam同一のo'], ['savoj', 'savoj', 'sav救助するoj'], ['senson', 'sen(~)なしでson音がする', 'sens[生]感覚on'], ['sepon', 'sep七on分数', 'sep七on'], ['skadron', 'skadron', 'skadr[軍]騎兵中隊on'], ['stadion', 'stadionスタジアム', 'stadi段階on'], ['tetraon', 'tetraon', 'tetraエゾライチョウon'], ['timon', 'timonかじ棒', 'tim恐れるon'], ['valon', 'valon', 'val[地]谷on'], ['veto', 'veto', 'vet賭けるo']]
overlap_6_c   =  [['arĝentan', 'arĝentan', 'arĝent銀an'], ['inka', 'inka', 'inkインクa'], ['platan', 'platan[植]プラタナス', 'plat平たいan'], ['dekan', 'dekan学部長', 'dek十an'], ['kalkan', 'kalkan[解]踵', 'kalk[化]石灰an'], ['fazan', 'fazan[鳥]キジ', 'faz[理]位相an'], ['havaj', 'havaj', 'hav持っているaj'], ['koran', 'koran[宗]コーラン', 'kor心an'], ['lama', 'lama[宗]ラマ僧', 'lamびっこのa'], ['malaj', 'malaj', 'mal正反対aj'], ['marian', 'marian', 'mariマリアan'], ['paran', 'paran', 'par一対an'], ['satan', 'satan[宗]サタン', 'sat満腹したan'], ['sudan', 'sudan', 'sud南an']]
overlap_7_c   =  [['alte', 'alteタチアオイ', 'alt高いe'], ['apoge', 'apoge[天]遠地点', 'apog(を)支えるe'], ['kaze', 'kaze[化]凝乳', 'kaz[文]格e'], ['pere', 'pere破滅する', 'perよってe'], ['kore', 'kore', 'kor心e'], ['male', 'male', 'mal正反対e'], ['sole', 'soleシタビラメ', 'sol唯一のe']]
overlap_8_c   =  [['regulus', 'regulus', 'regul規則us条件法'], ['akirant', 'akirant', 'akir(を)獲得するant能動;継続'], ['radius', 'radius', 'radi[理]線us条件法'], ['premis', 'premis前提', 'prem(を)押えるis過去形'], ['sonat', 'sonat[楽]ソナタ', 'son音がするat受動継続'], ['format', 'format[印]判', 'form形at受動継続'], ['markot', 'markot[園]取木', 'markしるしot受動将然'], ['nomad', 'nomad遊牧民', 'nom名前ad継続行為'], ['kantat', 'kantat[楽]カンタータ', 'kant(を)歌うat受動継続'], ['kolorad', 'kolorad', 'kolor色ad継続行為'], ['diplomat', 'diplomat外交官', 'diplom免状at受動継続'], ['diskont', 'diskont[商]手形割引する', 'disk円盤ont能動;将然'], ['endos', 'endos', 'end必要os未来形'], ['esperant', 'esperantエスペラント', 'esper(を)希望するant能動;継続'], ['forkant', 'for離れてkant(を)歌う', 'fork[料]フォークant能動;継続'], ['gravit', 'gravit', 'grav重要なit受動完了'], ['konus', 'konus[数]円錐', 'kon知っているus条件法'], ['salat', 'salat[料]サラダ', 'sal塩at受動継続'], ['legat', 'legat[宗]教皇特使', 'leg(を)読むat受動継続'], ['lekant', 'lekant[植]マーガレット', 'lekなめるant能動;継続'], ['lotus', 'lotus[植]ハス', 'lotくじus条件法'], ['malvolont', 'mal正反対volont自ら進んで', 'mal正反対vol意志があるont能動;将然'], ['mankis', 'man手kisキスする', 'mank欠けているis過去形'], ['minus', 'minusマイナス', 'min鉱山us条件法'], ['patos', 'patos[芸]パトス', 'patフライパンos未来形'], ['predikat', 'predikat[文]述部', 'predik(を)説教するat受動継続'], ['rabat', 'rabat[商]割引', 'rab強奪するat受動継続'], ['rabot', 'rabotかんなをかける', 'rab強奪するot受動将然'], ['remont', 'remont', 'rem漕ぐont能動;将然'], ['satirus', 'satirus', 'satir諷刺(詩;文)us条件法'], ['sendat', 'sen(~)なしでdat日付', 'send(を)送るat受動継続'], ['sendot', 'sen(~)なしでdot持参金', 'send(を)送るot受動将然'], ['spirit', 'spirit精神', 'spir呼吸するit受動完了'], ['spirant', 'spirant', 'spir呼吸するant能動;継続'], ['taksus', 'taksus[植]イチイ', 'taks(を)評価するus条件法'], ['tenis', 'tenis', 'ten支え持つis過去形'], ['traktat', 'traktat[政]条約', 'trakt(を)取り扱うat受動継続'], ['trikot', 'trikot[織]トリコット', 'trik編み物をするot受動将然'], ['trilit', 'tri三litベッド', 'tril[楽]トリルit受動完了'], ['vizit', 'vizit(を)訪問する', 'vizビザit受動完了'], ['volont', 'volont自ら進んで', 'vol意志があるont能動;将然']]
overlap_9_c   =  [['premi', 'premi賞品', 'prem(を)押えるi'], ['bari', 'bari', 'bar障害i'], ['tempi', 'tempiこめかみ', 'temp時間i'], ['noktu', 'noktu[鳥]コフクロウ', 'nokt夜u'], ['vakcini', 'vakcini', 'vakcin[薬]ワクチンi'], ['procesi', 'procesi[宗]行列', 'proces[法]訴訟i'], ['statu', 'statu立像', 'stat状態u'], ['devi', 'devi', 'devmusti'], ['feri', 'feri休日', 'fer鉄i'], ['fleksi', 'fleksi[文]語尾変化', 'fleks(を)曲げるi'], ['pensi', 'pensi年金', 'pens思うi'], ['jesu', 'jesu[宗]イエス', 'jesはいu'], ['konfesi', 'konfesi', 'konfes(を)告白するi'], ['konsili', 'konsili', 'konsil(を)助言するi'], ['legi', 'legi[史]軍団', 'leg(を)読むi'], ['licenci', 'licenci', 'licenc[商]認可i'], ['loĝi', 'loĝi[劇]桟敷', 'loĝ(に)住むi'], ['meti', 'meti手仕事', 'met(を)置くi'], ['misi', 'misi使命', 'mis誤りi'], ['pasi', 'pasi情熱', 'pas通過するi'], ['revu', 'revu専門雑誌', 'rev空想するu'], ['rabi', 'rabi[病]狂犬病', 'rab強奪するi'], ['religi', 'religi宗教', 're再びlig結ぶ;連盟i'], ['sagu', 'sagu[料]サゴ粉', 'sag矢u'], ['sekci', 'sekci部', 'sekc[医]切断するi'], ['sendi', 'sen(~)なしでdi神', 'send(を)送るi'], ['teni', 'teniサナダムシ', 'ten支え持つi'], ['vaku', 'vaku', 'vakあいているu'], ['vizi', 'vizi幻影', 'vizビザi']]
...
def compare_all():
    print("=== overlap_1 ===")
    pprint(compare_list_of_lists(overlap_1_a, overlap_1_c))
    print()
    
    print("=== overlap_2 ===")
    pprint(compare_list_of_lists(overlap_2_a, overlap_2_c))
    print()
    
    print("=== overlap_2_2 ===")
    pprint(compare_list_of_lists(overlap_2_2_a, overlap_2_2_c))
    print()
    
    print("=== overlap_2_3 ===")
    pprint(compare_list_of_lists(overlap_2_3_a, overlap_2_3_c))
    print()
    
    print("=== overlap_3 ===")
    pprint(compare_list_of_lists(overlap_3_a, overlap_3_c))
    print()
    
    print("=== overlap_4 ===")
    pprint(compare_list_of_lists(overlap_4_a, overlap_4_c))
    print()
    
    print("=== overlap_5 ===")
    pprint(compare_list_of_lists(overlap_5_a, overlap_5_c))
    print()
    
    print("=== overlap_6 ===")
    pprint(compare_list_of_lists(overlap_6_a, overlap_6_c))
    print()
    
    print("=== overlap_7 ===")
    pprint(compare_list_of_lists(overlap_7_a, overlap_7_c))
    print()
    
    print("=== overlap_8 ===")
    pprint(compare_list_of_lists(overlap_8_a, overlap_8_c))
    print()
    
    print("=== overlap_9 ===")
    pprint(compare_list_of_lists(overlap_9_a, overlap_9_c))
    print()
...
## 以下、'an'系、'on'系の処理。 'an'は形容詞語尾、'on'は名詞語尾であるが、「集団の構成員」を表す接尾辞'an'と「分数」を表す接尾辞'on'と重複することがあるため、専用の処理が必要になる。
...
with open("世界语单词词根分解方法の使用者自定义设置.json", 'r', encoding='utf-8') as f:
    text = f.read()
    converted_text = convert_to_circumflex(text)# テキストを小文字に変換し、字上符形式（ĉ, ĝ, ĥ, ĵ, ŝ, ŭなど）に統一。
    custom_stemming_setting_list = json.loads(converted_text)
...
# (漢字)置換後文字列のユーザー設定を読み込む。　置換後文字列のユーザー設定は、言語の一般性を失うので、基本的にオススメできない。
with open("替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json", 'r', encoding='utf-8') as f:
    text = f.read()
    converted_text = convert_to_circumflex(text)# テキストを字上符形式（ĉ, ĝ, ĥ, ĵ, ŝ, ŭなど）に統一。
    user_replacement_item_setting_list = json.loads(converted_text)
...
# エスペラント文の文字列(漢字)置換においては、"大文字"、"小文字"、"頭文字だけ大文字"、の3パターンの文字列(漢字)置換に対応できるように置換用の'replacements'リストを作成するが、"頭文字だけ大文字"に対応するには、
# HTMLタグをうまく避けながら、頭文字のみを大文字化しておく必要性がある。("大文字"の場合は、HTML形式の仕様上、HTMLタグごと大文字化しても全く問題ない。)　下記はそれ用の関数。
import re
...
# '大文字'、'小文字'、'文頭だけ大文字'の3パターンの文字列(漢字)置換に対応。
...
text = "hello <ruby>kaj<rt class=\"ruby-L_L_L\">and</rt></ruby>"# <ruby> の前に文字列がある。
result = capitalize_ruby_and_rt(text)
print("入力: ", text,"  出力: ", result)
text = "<ruby>kaj<rt class=\"ruby-L_L_L\">and</rt></ruby> world"# 行頭が <ruby> で始まる
result = capitalize_ruby_and_rt(text)
print("入力: ", text,"  出力: ", result)
text = "Before <ruby>abc<rt class=\"ruby-L_L_L\">def"# </ruby> が省略されている
result = capitalize_ruby_and_rt(text)
print("入力: ", text,"  出力: ", result)
text = "<ruby>kaj<rt class=\"ruby-L_L_L\">and</rt></ruby> <ruby>xyz<rt class=\"ruby-L_L_L\">zzz</rt></ruby>"# 1行に複数の <ruby> がある
result = capitalize_ruby_and_rt(text)
print("入力: ", text,"  出力: ", result)
text = "hello  <ruby>abc<rt class=\"ruby-XXS_S_S\">def</rt></ruby> <ruby>kaj<rt class=\"ruby-L_L_L\">and</rt></ruby> <ruby>xyz<rt class=\"ruby-L_L_L\">zzz</rt></ruby>"# 1行に複数の <ruby> がある
result = capitalize_ruby_and_rt(text)# ⇑が小文字化してしまう
print("入力: ", text,"  出力: ", result)
text = "tem<ruby>er<rt class=\"ruby-S_S_S\">粒子</rt></ruby>"# 1行に複数の <ruby> がある
result = capitalize_ruby_and_rt(text)# ⇑が小文字化してしまう
print("入力: ", text,"  出力: ", result)
# ##tem<ruby>er<rt class=\"ruby-S_S_S\">粒子</rt></ruby>
# <ruby>lu<rt class=\"ruby-xxs_s_s\">賃借<br>する</rt></ruby>i
# <ruby>ŝat<rt class=\"ruby-xxs_s_s\">価値を<br>認める</rt></ruby>
# "Tekokomput<ruby>il<rt class=\"ruby-xxs_s_s\">道<br>具</rt></ruby>",
text = "<ruby>lu<rt class=\"ruby-XXS_S_S\">賃借<br>する</rt></ruby>i"# 1行に複数の <ruby> がある
result = capitalize_ruby_and_rt(text)# ⇑が小文字化してしまう
print("入力: ", text,"  出力: ", result)
text = "tekokomput<ruby>il<rt class=\"ruby-XXS_S_S\">道<br>具</rt></ruby>"# 1行に複数の <ruby> がある
result = capitalize_ruby_and_rt(text)# ⇑が小文字化してしまう
print("入力: ", text,"  出力: ", result)
text = "<ruby>it<rt class=\"ruby-XXXS_S\">受動<br>完<br>了</rt></ruby>a"
result = capitalize_ruby_and_rt(text)# ⇑が小文字化してしまう
print("入力: ", text,"  出力: ", result)
text = "it <ruby>it<rt class=\"ruby-XXXS_S\">受動<br>完<br>了</rt></ruby>a"
result = capitalize_ruby_and_rt(text)# ⇑が小文字化してしまう
print("入力: ", text,"  出力: ", result)
...
imported_placeholders_for_local_replacement = import_placeholders('占位符(placeholders)_@20374@-@97648@_局部文字列替换用.txt')# placeholderに'@'が含まれる必要は全く無いが、雰囲気を揃えるために敢えて入れた。
...
# 以下では2文字語根用の文字列(漢字)置換処理を実装した。 通常、2文字語根単独の置換処理の実装は、本コードのように文字列置換に頼った語根分解では、置換精度の観点から難しいことが多いが、"置換対象の2文字語根が、**既にplaceholderに置換された文字列に隣接している場合**に限っては実装可能なのではないか"と考えた。

# In[47]:


# 接尾語として使用されうる2語根、接頭語として使用されうる語根、単体で使用されうる語根に分類。重複も多数ある。
...
imported_placeholders_for_2char_replacement = import_placeholders('占位符(placeholders)_$13246$-$19834$_二文字词根替换用.txt')# 文字列(漢字)置換時に用いる"placeholder"ファイルを予め読み込んでおく。
...
# #### __エスペラント文の文字列(漢字)置換のために最小限実行する必要がある範囲２(始まり)__
# (最後の実際にエスペラント文を文字列(漢字)置換するセルまで)
...
def orchestrate_comprehensive_esperanto_text_replacement(
    ...
    text = convert_to_circumflex(text,hat_to_circumflex)# テキストを字上符形式（ĉ, ĝ, ĥ, ĵ, ŝ, ŭなど）に統一。
...
def parallel_process(text: str, num_processes: int = num_processes, 
    ...
    if num_lines <= 1:# 並列化の意味がほぼないので、そのまま処理
        return process_segment(lines, placeholders_for_skipping_replacements,replacements_list_for_localized_string,placeholders_for_localized_replacement,replacements_final_list,replacements_list_for_2char,format_type)
...
format_type_='HTML格式_Ruby文字_大小调整'

# format_type='HTML格式_Ruby文字_大小调整'
with open('例句_Esperanto文本.txt','r', encoding='utf-8') as g:
    text0=g.read()

text1=orchestrate_comprehensive_esperanto_text_replacement(text0, format_type=format_type_)
# orchestrate_comprehensive_esperanto_text_replacement(
#     text0, placeholders_for_skipping_replacements, replacements_list_for_localized_string, 
#     placeholders_for_localized_replacement, replacements_final_list, 
#     replacements_list_for_2char, format_type)

text1 = apply_ruby_html_header_and_footer(text1, format_type_)

with open('Esperanto_Text_Replacement_Result.html','w', encoding='utf-8') as h:
    h.write(text1)


# #### __エスペラント文の文字列(漢字)置換のために最小限実行する必要がある範囲２(終わり)__
...
text1=parallel_process(text0, num_processes=8, format_type = format_type_)
...
# 上記で一番処理に時間がかかるセルについて、multiprocessingによる並列化を実装した。 品詞の並ぶ順番は違うものの、正しく実行できていることは確認済み。 # 我已经对上述最耗时的处理进行了并行化。尽管词性的顺序有所不同，但我已经确认它正确执行。import json
import math
import multiprocessing
...
# 質問文にある safe_replace(...) をそのまま利用する想定。
# placeholders(占位符)を用いた置換。
def safe_replace(text: str, replacements: List[Tuple[str, str, str]]) -> str:
    """
    :param text: 置換対象の文字列
    :param replacements: [ (old, new, placeholder), ... ] の形式
    :return: 置換後の文字列
    """
    valid_replacements = {}
    # まず old → placeholder に一時置換
    for old, new, placeholder in replacements:
        if old in text:
            text = text.replace(old, placeholder)
            valid_replacements[placeholder] = new
    # 次に placeholder → new に最終置換
    for placeholder, new in valid_replacements.items():
        text = text.replace(placeholder, new)
    return text

# temporary_replacements_list_final は、
# [ (old, new, placeholder), (old, new, placeholder), ... ] というリストを想定。
with open("一时的な替换用のリスト(列表)型配列(temporary_replacements_list_final).json", "r", encoding="utf-8") as f:
    temporary_replacements_list_final = json.load(f)

# 例えば E_stem_with_Part_Of_Speech_list が
# [
#   ["labor", "v"],
#   ["labor", "n"],
#   ["nom",   "n"],
...
########################################
# 3) 並列実行で部分辞書を作り、マージして最終的な pre_replacements_dict_1 を作る関数
########################################
def parallel_build_pre_replacements_dict(
    ...
    chunk_size = math.ceil(total_len / num_processes)
    chunks = []
    start_index = 0
...
########################################
# 4) 実際にメイン部で呼び出して JSON 保存 (サンプル)
########################################
if __name__ == "__main__":
    # 例: "PEJVO(世界语全部单词列表)'全部'について..." などの JSON を読み込み
    # E_stem_with_Part_Of_Speech_list: [[E_root, pos], ...]
    with open("PEJVO(世界语全部单词列表)'全部'について、词尾(a,i,u,e,o,n等)をcutし、comma(,)で隔てて词性と併せて记录した列表(E_stem_with_Part_Of_Speech_list).json", "r", encoding="utf-8") as f:
        E_stem_with_Part_Of_Speech_list = json.load(f)

    # プロセス数 (4コア/8コアなど、CPUに合わせて)
    NUM_PROCESSES = 4
    ...
    # 結果(辞書)を確認 (例として最初の10件だけ print)
    sample_items = list(pre_replacements_dict_1.items())[:10]
    for k, v in sample_items:
        print(k, ":", v)

    # JSON に保存
    with open("替换用の辞書(字典)型配列(pre_replacements_dict_1).json", "w", encoding="utf-8") as g:
        json.dump(pre_replacements_dict_1, g, ensure_ascii=False, indent=2)

    print("JSONファイルへ保存完了:", "替换用の辞書(字典)型配列(pre_replacements_dict_1).json")
...
# 上記で一番処理に時間がかかるセルについて、multiprocessingによる並列化を実装した。 品詞の並ぶ順番は違うものの、正しく実行できていることは確認済み。 # 我已经对上述最耗时的处理进行了并行化。尽管词性的顺序有所不同，但我已经确认它正确执行。import json
# 以下は上記のコードに沢山説明を付けたコード。実装機能、出力結果は全く同じである。
...
def safe_replace(text: str, replacements: List[Tuple[str, str, str]]) -> str:
    """
    placeholders(占位符) を使った安全な置換を行う関数です。
    
    【引数】
    - text:  置換対象の元テキスト。
    - replacements: リスト形式 [(old, new, placeholder), ...] で渡される置換ルール。
                    "old" → "placeholder" → "new" という二段階置換で衝突を防ぎます。
...
# 上記の safe_replace(...) が使う置換リストを JSON からロードします。
#   形式は [(old, new, placeholder), (old, new, placeholder), ...] を想定。
with open("一时的な替换用のリスト(列表)型配列(temporary_replacements_list_final).json", "r", encoding="utf-8") as f:
    temporary_replacements_list_final = json.load(f)

# 以下の E_stem_with_Part_Of_Speech_list は、
# [
#   ["labor", "v"],
#   ["labor", "n"],
#   ["nom",   "n"],
...
# ]
# のような2次元配列を想定。
# これも後で同様に JSON からロードします。

########################################
# 2) 並列実行するための下請け関数
########################################
def process_chunk_for_pre_replacements(
    subset_of_E_stems: List[List[str]],
    replacements: List[Tuple[str, str, str]]
) -> Dict[str, List[str]]:
    """
    並列で実行される部分処理を担う関数。
    ...
    【引数】
    - subset_of_E_stems: E_stem_with_Part_Of_Speech_list の一部分(チャンク)。
                        要素は [E_root, part_of_speech] 形式を想定。
    - replacements: safe_replace(...) に渡す置換リスト (temporary_replacements_list_final)

    【処理概要】
    - E_root(エスペラント語根)が2文字以上のものについてのみ、safe_replace(...) で置換を実行。
    - 既に同じ E_root があれば品詞( part_of_speech )を","結合で追記。

    【戻り値】
    - 以下の形式の辞書を返す。
      {
         E_root文字列: [ safe_replace 後の文字列, 品詞をカンマ区切りにした文字列 ],
         ...
      }
    """
    local_result_dict = {}

    for item in subset_of_E_stems:
        # item は [E_root, pos] の2要素リストを想定
        if len(item) != 2:
            # 万が一形式が崩れている場合はスキップ
            continue
        
        E_root, part_of_speech = item

        # 元コードのロジックでは、2文字未満はスキップ (3文字以上にするなどの調整は任意)
        if len(E_root) < 2:
            continue

        # 既に local_result_dict に存在する E_root であれば、
        # 品詞部分のみ結合して追記する。
        if E_root in local_result_dict:
            replaced_stem, existing_pos_str = local_result_dict[E_root]
            existing_pos_list = existing_pos_str.split(',')

            # 新しい品詞がまだ含まれていない場合だけ追加
            if part_of_speech not in existing_pos_list:
                existing_pos_list.append(part_of_speech)
                merged_pos_str = ",".join(existing_pos_list)
                local_result_dict[E_root] = [replaced_stem, merged_pos_str]

        else:
            # === safe_replace で置換を実行 (今回のコア処理) ===
            replaced_text = safe_replace(E_root, replacements)
            local_result_dict[E_root] = [replaced_text, part_of_speech]

    return local_result_dict


########################################
# 3) 並列実行で部分辞書を作り、マージして最終 dict を作る関数
########################################
def parallel_build_pre_replacements_dict(
    E_stem_with_Part_Of_Speech_list: List[List[str]],
    replacements: List[Tuple[str, str, str]],
    ...
    """
    E_stem_with_Part_Of_Speech_list を複数プロセスに分割し、
    process_chunk_for_pre_replacements(...) を並列実行して集約する関数。

    【引数】
    - E_stem_with_Part_Of_Speech_list: [[E_root, part_of_speech], [E_root, part_of_speech], ...]
    - replacements: safe_replace(...) に渡す置換ルールのリスト
    - num_processes: 同時実行したいプロセス数 (CPUコア数に応じて調整)

    【戻り値】
    - 以下の形式の辞書を返す。
      {
         E_root文字列: [ safe_replace 後の文字列, 品詞がカンマ区切りに連なった文字列 ],
         ...
      }
    """
    total_len = len(E_stem_with_Part_Of_Speech_list)
    ...
    # === (A) 入力リストを num_processes 個のチャンクに分割 ===
    chunk_size = math.ceil(total_len / num_processes)
    list_of_chunks = []
    start_index = 0
    ...
    # === (B) 各プロセスが process_chunk_for_pre_replacements(...) を実行 ===
    with multiprocessing.Pool(num_processes) as pool:
        partial_result_dicts = pool.starmap(
            process_chunk_for_pre_replacements,
            [(chunk, replacements) for chunk in list_of_chunks]
        )

    # partial_result_dicts は、
    #   [
    #     {E_root1: [replaced_stem, "n,v"], E_root2: [...], ...},  # プロセス1の結果
    #     {E_root2: [...], ...},                                  # プロセス2の結果
    #     ...
    #   ]
    # のように複数の辞書が入ったリスト。
    ...
    for partial_dict in partial_result_dicts:
        for e_root, val in partial_dict.items():
            replaced_stem, pos_str = val

            # まだ merged_result_dict にキーが無ければ新規追加
            if e_root not in merged_result_dict:
                merged_result_dict[e_root] = [replaced_stem, pos_str]
            else:
                # すでに存在する E_root の品詞だけを統合
                existing_replaced_stem, existing_pos_str = merged_result_dict[e_root]
                
                # replaced_stem が異なるケースは基本起こらない想定だが、
                # どちらを優先するか等の処理を入れるならここで行う。
                
                # 品詞文字列を合体して重複を除去
                existing_pos_list = existing_pos_str.split(',')
                new_pos_list = pos_str.split(',')
                merged_set = set(existing_pos_list) | set(new_pos_list)
                merged_pos_list = sorted(list(merged_set))  # 必要ならソート
                merged_pos_str = ",".join(merged_pos_list)

                # replaced_stem は先行登録のものを残す (必要なら衝突処理を加える)
                merged_result_dict[e_root] = [existing_replaced_stem, merged_pos_str]

    return merged_result_dict


########################################
# 4) 実際のメイン部で呼び出して JSON 保存 (サンプル)
########################################
if __name__ == "__main__":
    # === 事前に用意されている JSON から E_stem_with_Part_Of_Speech_list をロード ===
    with open(
    ...
    # 結果(辞書)の先頭10件をサンプル表示
    items_sample = list(pre_replacements_dict_1.items())[:10]
    for e_root_key, val in items_sample:
        print(f"{e_root_key} : {val}")

    # 生成した辞書を JSON に保存
    output_json = "替换用の辞書(字典)型配列(pre_replacements_dict_1).json"
    with open(output_json, "w", encoding="utf-8") as g:
        json.dump(pre_replacements_dict_1, g, ensure_ascii=False, indent=2)

    print("JSONファイルへ保存完了:", output_json)
